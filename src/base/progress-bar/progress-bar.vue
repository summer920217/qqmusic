...
export default {
  ...
  methods: {
    ...
    _offSet(distance){
      ...
      //修改小圆点的位置
      this.$refs.progressBtn.style.left = distance-15 + "px";
    },
    //开始点击
    touchstart(e){
      //设置一个开关，记录触摸事件是否开始
    ...
    touchmove(e){
      //按下去的时候才可以进行拖动操作
      if(!this.touches.start){//start为false，表示touchend事件已经触发执行了，说明鼠标已经松开了
        return;
      }
      //鼠标按下没松开
      ...
      if(distance>this.$refs.progressBar.clientWidth){distance = this.$refs.progressBar.clientWidth}
